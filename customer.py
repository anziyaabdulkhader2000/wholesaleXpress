...
def customer_viewproduct():
	...
	if "search" in request.form:
		...
		q="SELECT * FROM product  INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN `brand` USING (brand_id) where  brand_name like '%s' and brand_status='1' or subcategory_name like '%s'  and sub_status='1' or product_name like '%s' and product_status='1' group by product_id "%(p,p,p)
		res=select(q)
		...
		q="SELECT * FROM product   INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN `category` USING (`category_id`) INNER JOIN `brand` USING (brand_id)   where product_status='1'  and brand_status='1' and sub_status='1' and status='1'  and rate<> 0 GROUP BY product_id  "
	res=select(q)
	data['productsearch']=res
...
def customer_viewcart():
	...
	q="SELECT * FROM `order_details` INNER JOIN `order_master` USING (`order_master_id`) INNER JOIN `product` USING (`product_id`) INNER JOIN `customer` USING (customer_id)  INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN brand USING (brand_id) where order_status='pending' and customer_id='%s'"%(cid)
	res=select(q)
	data['len']=len(res)
	data['cart']=res
	...
	if action=='cashondelivery':
		q="UPDATE `order_master` SET `order_status`='cashondelivery' WHERE `order_master_id`='%s'"%(order_master_id)
		update(q)
...
def customer_viewmyorder():
	...
	q="SELECT * FROM `order_details` INNER JOIN `order_master` USING (`order_master_id`) INNER JOIN `product` USING (`product_id`) INNER JOIN `customer` USING (customer_id)  INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN brand USING (brand_id) where  customer_id='%s' and order_status!='pending'  "%(cid)
	res=select(q)
	data['myorder']=res
...
def customerprofile():
    ...
    if "cusregss" in request.form:
    	...
    	n=request.form['num']
    	q="update customer set customer_fname='%s' ,customer_lname='%s',customer_houser_name='%s',customer_street='%s',customer_dist='%s',customer_state='%s',customer_pincode='%s',customer_phone='%s' where customer_id='%s'"%(f,l,h,s,di,st,pi,n,cid)
    	update(q)
