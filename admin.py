...
def admin_managevendor():
	...
	if "vendor" in request.form:
		...
		if res:
			...
			insert(q)
			q="insert into vendor values(null,'%s','0','%s','%s','%s','%s','%s','%s','%s','%s','1')"%(u,f,n,e,h,s,di,p,d)
			insert(q)
			flash('successfully')
			return redirect(url_for('admin.admin_managevendor'))
...
def admin_manageitem():
	...
	q="SELECT * FROM product  INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN brand USING (brand_id)"
	res=select(q)
	...
	if action=='update':
		q="SELECT * FROM product   INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN brand USING (brand_id) where product_id='%s'"%(pid)
		res=select(q)
	...
	if "update" in request.form:
		...
		if request.files['imgg']:
			...
			i.save(path)	
			q="update product set subcategory_id='%s',color_id='0',brand_id='%s',product_name='%s',product_description='%s',product_image='%s',pro_per='%s' where product_id='%s'"%(sub,b,f,d,path,pro,pid)
		else:
			q="update product set subcategory_id='%s',color_id='0',brand_id='%s',product_name='%s',product_description='%s',pro_per='%s' where product_id='%s'"%(sub,b,f,d,pro,pid)
	...
	if "product" in request.form:
		...
		if res:
			...
			q="insert into product values(null,'%s','0','%s','%s','%s','%s','%s','%s','0','1','0')"%(sub,b,f,d,path,pro,exp)
			insert(q)
			flash('successfully')
			return redirect(url_for('admin.admin_manageitem'))		
...
def admin_viewbooking():
	data={}
	q="SELECT * FROM `order_details`  INNER JOIN `order_master` USING (`order_master_id`) INNER JOIN `product` USING (`product_id`) INNER JOIN `customer` USING (customer_id)  INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN brand USING (brand_id) where order_status='Delivered' or order_status='Paid' "
	res=select(q)
...
def admin_managereport():
	...
	if "sale" in request.form:
		...
		q="SELECT * FROM `order_details` INNER JOIN `order_master` USING (`order_master_id`) INNER JOIN `product` USING (`product_id`) INNER JOIN `customer` USING (customer_id)  INNER JOIN `subcategory` USING (`subcategory_id`) INNER JOIN brand USING (brand_id) "
		res=select(q)
		data['report']=res
